...
@pytest.mark.parametrize("tle_age_limit", list(TleAgeLimit))
def test_validation_accepts_supported_tle_age_limits(
    station_factory,
    search_window_factory,
    search_criteria_factory,
    tle_age_limit,
    ...
    from tlefinder.core.models import SatelliteGroup, SearchRequest
    from tlefinder.core.validation import validate_search_request, validate_tle_age_limit
    ...
    request = SearchRequest(
        ...
        tle_age_limit=tle_age_limit,
    )

    validate_tle_age_limit(tle_age_limit)
    validate_search_request(request)
...
@pytest.mark.parametrize(
    "tle_age_limit",
    [
        pytest.param("24h", id="raw-string"),
        pytest.param(None, id="none"),
    ],
)
def test_validation_rejects_invalid_tle_age_limit_values(
    station_factory,
    search_window_factory,
    search_criteria_factory,
    tle_age_limit,
    ...
    with pytest.raises(ValidationError, match="tle_age_limit"):
        validate_search_request(request)


def test_validation_rejects_unsupported_enum_like_tle_age_limit(
    station_factory,
    search_window_factory,
    search_criteria_factory,
):
    ...
    class UnsupportedTleAgeLimit(str, Enum):
        MONTH_1 = "1m"
    ...
    request = SearchRequest(
        ...
        criteria=search_criteria_factory(),
        satellite_group=SatelliteGroup.ACTIVE,
        tle_age_limit=UnsupportedTleAgeLimit.MONTH_1,
    )
