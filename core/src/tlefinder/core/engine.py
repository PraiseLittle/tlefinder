...
TLE_ONE_WEEK_MAX_AGE_HOURS = 168
...
def search_candidates(
    ...
    *,
    ...
    with _record_stage_timing(timings_ms, "tle_loading", timer_func):
        effective_max_tle_age_hours = (
            max_tle_age_hours
            if max_tle_age_hours is not None
            else _max_tle_age_hours_for_request(request.tle_age_limit)
        )
        records = tle_repository.load_tle_dataset(
            ...
            **_tle_repository_kwargs(
                ...
                max_tle_age_hours=effective_max_tle_age_hours,
            ),
        )
...
def _max_tle_age_hours_for_request(tle_age_limit: TleAgeLimit) -> int | None:
    if tle_age_limit is TleAgeLimit.WEEK_1:
        return TLE_ONE_WEEK_MAX_AGE_HOURS
    return None
