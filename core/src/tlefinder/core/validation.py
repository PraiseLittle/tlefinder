...
SUPPORTED_TLE_AGE_LIMITS = frozenset(
    {
        TleAgeLimit.HOURS_24,
        TleAgeLimit.WEEK_1,
    }
)
...
def validate_search_request(request: SearchRequest) -> None:
    ...
    validate_tle_age_limit(request.tle_age_limit)
...
def validate_tle_age_limit(tle_age_limit: TleAgeLimit) -> None:
    """Validate that ``tle_age_limit`` is one of the supported freshness policies."""

    if (
        not isinstance(tle_age_limit, TleAgeLimit)
        or tle_age_limit not in SUPPORTED_TLE_AGE_LIMITS
    ):
        supported = ", ".join(
            supported_limit.value
            for supported_limit in sorted(
                SUPPORTED_TLE_AGE_LIMITS,
                key=lambda supported_limit: supported_limit.value,
            )
        )
        raise ValidationError(f"tle_age_limit must be one of: {supported}")
